...
**現状は内部制作・比較用のCLIです。実OSVの5位置・10画像からGPU生成、候補比較、判定の保存・復元まで確認しましたが、両候補とも設備の位置・周辺形状を説明する用途では画質FAILです。SOG変換、衝突形状、ブラウザ歩行、FieldSplat接続は未検証・未実装です。歩行アプリの完成版ではありません。** ダミー生成データを復元成功として扱う機能はありません。
...
2026-09-26のChrome受入では、10,000反復・199,952 Gaussianと20,000反復・197,937 Gaussianを各8視点で確認し、両候補とも「設備の位置・周辺形状を明瞭に把握・説明する」用途でFAILでした。確認者はCodexの画面観察で、顧客検収ではありません。同じ現場の8視点×2候補です。M3 Max開発PCでA/B切替・16件の実ファイル保存と取り込み・再起動復元・読み込み復旧を確認済みです。[証拠と条件](docs/review-3b.md)を参照してください。別担当者・別PC・通信遮断・顧客納品はNOT_TESTEDです。

マスクなしのジョブも内部プレビューでき、未適用と明示します。視点・候補を変えると判定は未確認に戻り、再確認してからJSONを書き出します。過去の判定の復元は、新しい判定と分けて扱います。
...
承認済みのextract/mask/sfmを検査して独立コピーし、学習だけを新たに実行します。親ジョブや画質判定は書き換えません。初版は反復数の変更に限定します。両候補のプレビューを作成後、`preview-compare PREVIEW_A PREVIEW_B work/compare-01`で同じSfM・表示条件を確認してA/B比較できます。[操作と比較条件](docs/preview.md)、[検証範囲](docs/review-3b.md)を参照してください。

## 現場PoCの準備

動画がない段階でも`field-plan --before-capture`で対象・必要情報を定め、印刷用HTMLを作れます。撮影後は`field-link`で処理前の新規ジョブへ原案を保全して関連付けます。`field-report`は根拠写真の現在の有効性を再照合し、日本語の警告と有効な画像へのリンクを表示します。当時の撮影済み・公開確認と現在の根拠の状態は分離します。

`capture-manifest`で秒数・採用fpsから移動撮影の下書きを作成し、`field-plan`で必須箇所の内部確認表、`field-check`で根拠付き確認、`field-log`で実働と機械待ちを記録できます。`preflight`は画像数・全対候補数・段階内コピー量・空き容量を表示します。暫定予算は300画像・44,850候補・空き1GiBで、性能保証ではありません。超過時は理由付きの`budget`変更が必要です。未知の出力容量・GPU時間はUNKNOWNです。

[撮影・評価素材の分離、コマンド、業務比較と工数の記録手順](docs/field-poc.md)を参照してください。新しい静的な1室での画質・現場便益・採算はNOT_TESTEDです。
...
各段階の成果物を実際に確認した後、確認内容と根拠を記録します。承認時に成功時ハッシュと読み取り専用の形式検査を照合します。消失・破損・差し替えは拒否し、理由を保存します。成功時ハッシュがない旧試行は、新規ジョブで再実行してください。次の段階はこの記録がある場合に進みます。
...
- 同じ段階の途中から再開する機能はありません。学習反復数だけの変更は`derive`で上流を再利用できます。それ以外の設定変更は別の新規ジョブが必要です。
